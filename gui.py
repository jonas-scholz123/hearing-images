...
class App(Tk):
    def __init__(self):
        ...
        self.btn_begin_cap = Button(self, text="Begin Capture", command=self.begin_capture)
        ...
        self.btn_begin_cap.grid(column = 3, row = 3)
        self.btn_end_cap.grid(column = 3, row = 4)
        self.btn_takeim.grid(column = 4, row = 0)

        self.progress=Progressbar(self, orient=HORIZONTAL, length=150, mode='determinate')
        self.progress.grid(column = 0, row = 1, padx = 10, columnspan = 2)
    ...
    def convert_image(self, im = None):
        ...
        def real_convert_image():
            ...
            self.converter = Converter(self.method, self.image_path)
